...
def feature_scaling(data) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    mean, std = torch.mean(data_t, dim = 0), torch.std(data_t, dim=0, unbiased=False)
    data_standardized = (data_t - mean) / std
    ...
    x_min = torch.min(data_t, dim=0).values
    x_max = torch.max(data_t, dim=0).values

    data_minmax = (data_t- x_min) / (x_max - x_min)
